...
resource "local_file" "tfvars_snippet" {
  file_permission = "0644"
  filename        = "${path.module}/../tfstate.auto.tfvars"

  content = <<-EOT
  # Auto-generated by init bootstrap — the tfstate bucket name
  tfstate_bucket_name = "${google_storage_bucket.backend.name}"
  EOT
}
