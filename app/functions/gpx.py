...
def load_additional_gpx_files(directory):
    ...
    for file_path in gpx_files:
        try:
            with Path.open(file_path, encoding="utf-8") as gpx_file:
                print(f"Loading {gpx_file}")
                gpx_string = gpx_file.read()
                gpx_data = gpxpy.parse(gpx_string)
                file_name = Path.name(file_path)
...
def handle_uploaded_gpx(
    world_wide_hikes_path, skane_other_files_path, uploaded_file, is_world_wide=False
    ...
    try:
        ...
        with Path.open(tmp_file_path, encoding="utf-8") as test_file:
        ...
        Path.unlink(tmp_file_path)  # Remove the temporary file
        ...
        if Path.exists(tmp_file_path):
            Path.unlink(tmp_file_path)  # Clean up temp file if it exists
